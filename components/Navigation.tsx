      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex min-h-16 items-center justify-between gap-4 py-2">
          <div className="flex min-w-0 items-center gap-5">
            ...
            <div className="hidden items-center gap-5 text-sm font-semibold text-zinc-600 dark:text-zinc-300 md:flex">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="rounded-md px-1 py-1 transition-colors hover:text-zinc-900 dark:hover:text-zinc-50"
                >
                  {link.label}
                </Link>
              ))}
